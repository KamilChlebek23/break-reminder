...
- **PyInstaller's `--collect-data tzdata`** is required because `tzdata` is a data-only package (no Python modules to discover, only `.zoneinfo` IANA files) and PyInstaller bundles non-Python data files only when told to. Without the flag the packaged `.exe` silently ships without zone data, and the first `ZoneInfo("Europe/Warsaw")` call at runtime raises `ZoneInfoNotFoundError`. The failure mode is invisible to local dev and CI — `uv sync` installs `tzdata` directly into the venv where stdlib `zoneinfo` finds it. A contributor copying older 2024 PyInstaller templates may silently strip the flag; pin it in `release.yml` and mirror it in the local-build snippet in `README.md` and the inline comment in `pyproject.toml`. Surfaced by `context/archive/2026-06-02-bugfix-reminder-dst-drift/`; codified in `context/foundation/lessons.md` "PyInstaller data-only packages need `--collect-data`".
...
| `tzdata` data files missing from PyInstaller bundle breaks IANA timezone resolution → recurring reminders drift across DST | Unknown unknowns | Low | High | Add `--collect-data tzdata` to the PyInstaller invocation in `release.yml` (already in place since `bugfix-reminder-dst-drift`); mirror in the local-build snippet in `README.md` and the inline comment in `pyproject.toml`. Failure mode is invisible to local dev + CI (`uv sync` installs `tzdata` into the venv where stdlib `zoneinfo` finds it); only the packaged `.exe` exhibits it. Optional defense in depth: extend the existing `main.py --self-test` to exercise the data path (e.g. `ZoneInfo("UTC")`) so the release workflow's `--self-test` step catches a stripped flag. |
