...
| S-11 | bugfix-reminder-dst-drift | have recurring custom reminders fire at the intended wall-clock time across DST transitions | — | FR-014 | done |
...
### S-11: bugfix-reminder-dst-drift

- **Outcome:** custom reminders configured to recur daily / weekly / monthly fire at the user's intended wall-clock time (e.g. "Daily 9:00 Warsaw") across DST transitions instead of drifting by the DST offset on the first post-transition firing. The scheduler localizes `dtstart` and `now` to a new `Reminder.tz` IANA name before handing them to `rrulestr`, so RRULE walks the IANA calendar and respects DST. The Add / Edit Reminder form captures `tzlocal.get_localzone_name()` at save time; pre-S-11 `reminders.json` files lazy-migrate to OS-local at load (no manual migration). Fixes the observed defect where a "Daily 9:00 Warsaw" reminder set in standard time silently started firing one hour late on the spring-forward Sunday and kept that drift until the next transition.
- **Change ID:** `bugfix-reminder-dst-drift`
- **PRD refs:** FR-014 (recurrence is the surface the bug lives on); also touches the "Settings persist across reboots and updates" guardrail (the lazy-migration of pre-S-11 `reminders.json` files honors it — users do not lose data on the upgrade).
- **Prerequisites:** — (orthogonal to S-01..S-10; shipped on S-08's recurrence surface but doesn't require any other slice)
- **Parallel with:** —
- **Blockers:** —
- **Unknowns:** —
- **Risk:** low. Three-module change (`break_reminder/scheduler.py` for the localize + `_resolve_zone` helper, `break_reminder/storage/reminders.py` for the additive `Reminder.tz` field + `_coerce_tz`, `break_reminder/ui/reminder_form_dialog.py` for the form-side `tzlocal.get_localzone_name()` capture with conditional preserve-on-edit). Two new runtime deps (`tzdata` for stdlib `zoneinfo` data files; `tzlocal` for OS-local IANA detection) plus a `--collect-data tzdata` PyInstaller flag (the failure mode is invisible to local dev and CI — only the packaged `.exe` exhibits it; see `lessons.md` "PyInstaller data-only packages need `--collect-data`"). 20 new tests across `tests/test_reminders.py` (`TestCoerceTz` / `TestReminderTzField` / `TestRowContainmentForInvalidTz`), `tests/test_scheduler.py` (`TestDstDrift`), and `tests/test_reminder_form_dialog.py` (`TestReminderFormDialogTimezoneCapture`). Critical gotcha pinned in the plan: the regression test's `now` value MUST traverse the DST boundary between `start_at` and the firing under test (a `now` on the same side of the boundary as `start_at` cannot detect the bug — caught at the empirical-verification step during Phase 2).
- **Status:** done
...
| S-11 | `bugfix-reminder-dst-drift` | Localize RRULE math to a per-Reminder IANA timezone so recurring firings respect DST | yes | Discovered 2026-06-01 (R-1b TODO breadcrumb surfaced during the R-1 integration-test rollout); planned + shipped 2026-06-02 |
...
- **S-11: custom reminders configured to recur daily / weekly / monthly fire at the user's intended wall-clock time (e.g. "Daily 9:00 Warsaw") across DST transitions instead of drifting by the DST offset on the first post-transition firing. The scheduler localizes `dtstart` and `now` to a new `Reminder.tz` IANA name before handing them to `rrulestr` (so RRULE walks the IANA calendar); the Add / Edit Reminder form captures `tzlocal.get_localzone_name()` at save time; pre-S-11 `reminders.json` files lazy-migrate to OS-local at load.** — Archived 2026-06-02 → `context/archive/2026-06-02-bugfix-reminder-dst-drift/`. Lesson: localize RRULE math to an IANA-named timezone before `rrulestr` — a UTC-anchored `dtstart` walks the UTC calendar and recurring firings drift by the DST offset on every post-transition firing (captured in `lessons.md` as "RRULE math against a UTC-anchored `dtstart` drifts across DST"); separately, PyInstaller data-only deps like `tzdata` need an explicit `--collect-data` flag because the failure mode is invisible to local dev and CI (captured in `lessons.md` as "PyInstaller data-only packages need `--collect-data`"); and pin the regression test's `now` value to a value that **traverses** the DST boundary, not one on the same side of it — a same-side `now` cannot detect the bug, which is why R-1b shipped undetected through S-08's plan.
...
- **S-08: when adding or editing a reminder, the user can pick "Recurrence: none / daily / weekly / monthly" with a sensible default and an optional end date; the saved reminder fires on the configured cadence indefinitely (or until end date), proven by `next_firing_after()` advancing across firings.** — Archived 2026-05-28 → `context/archive/2026-05-28-reminders-recurrence-editor/`. Lesson (added retrospectively 2026-06-02): the slice's plan claimed "RRULE handles DST correctly" without qualification, but that claim was scoped only to the `_local_date_to_utc_end_of_day` end-date conversion — the recurring-firing path against a UTC-anchored `dtstart` silently drifted across every DST transition. Shipped undetected; surfaced four days later as S-11 / `bugfix-reminder-dst-drift` (see `context/archive/2026-06-02-bugfix-reminder-dst-drift/` and `lessons.md` "RRULE math against a UTC-anchored `dtstart` drifts across DST"). Future recurrence slices must verify "DST correctness" claims against a cross-DST simulation (a `now` that traverses the boundary between `start_at` and the firing under test), not just a local-time round-trip test that happens to live on one side of a DST transition.
