...
| Validation strategy | `QSpinBox.setMinimum(1)` / `setMaximum(240)` at the persistence boundary; transient `QToolTip` feedback on out-of-range typed entry (via `lineEdit.textEdited` capture) at the UX layer | Widget-level bounds are still load-bearing for the save path. UX feedback added during Phase 2 smoke after user reported silent revert/truncate; corrected to `textEdited` capture during impl-review when empirical testing showed `editingFinished` sees only post-fixup state. See plan.md "Addenda — 2026-05-25". | Plan + 2026-05-25 addendum |
