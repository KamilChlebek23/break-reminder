...
- [x] 2.1 Updated tests pass: `uv run pytest tests/test_app.py` — e3c16d2
- [x] 2.2 Full suite still passes: `uv run pytest` — e3c16d2
- [x] 2.3 Type check passes: `uv run pyright break_reminder/app.py tests/test_app.py` — e3c16d2
- [x] 2.4 Lint passes: `uv run ruff check break_reminder/app.py tests/test_app.py` — e3c16d2
- [x] 2.5 Format check passes: `uv run ruff format --check break_reminder/app.py tests/test_app.py` — e3c16d2

#### Manual

- [x] 2.6 The "Open settings…" tray menu item opens the new modal dialog (NOT the `QMessageBox` placeholder). — e3c16d2
- [x] 2.7 A left-click on the tray icon also opens the new dialog. — e3c16d2
- [x] 2.8 The spinbox is pre-populated with the current `break_interval_min` value (cross-checked against `BreakReminder.ini`). — e3c16d2
- [x] 2.9 Editing the spinbox to a new value and clicking OK closes the dialog and persists the value to the INI. — e3c16d2
- [x] 2.10 Within ≤1 second of clicking OK, the tray tooltip's countdown reflects the new interval. — e3c16d2
- [x] 2.11 Clicking Cancel after editing closes the dialog and leaves the INI value unchanged. — e3c16d2
- [x] 2.12 Quitting and restarting the app persists the last saved value. — e3c16d2
- [x] 2.13 The spinbox does not allow values below 1 or above 240 at the widget level. — e3c16d2
