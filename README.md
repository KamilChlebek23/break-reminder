...
API documentation for the `break_reminder` package is published to GitHub
Pages and auto-rebuilt on every push to `master`:

> https://kamilchlebek23.github.io/break-reminder/

Generated by [pdoc](https://pdoc.dev/) from the Google-style docstrings;
see [`.github/workflows/docs.yml`](.github/workflows/docs.yml) for the
build pipeline (and the exact CLI to reproduce it locally). `main.py` and
`tests/` are deliberately excluded — the published surface is the
package's public API only.
